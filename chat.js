...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: planCfg.model, messages, max_tokens: planCfg.maxTokens, temperature: planCfg.temp })
    });

    if (!response.ok) {
      const err = await response.json();
      return res.status(response.status).json({ reply: "Error de OpenAI: " + (err.error?.message || "desconocido") });
    }
