...
export default async function handler(req, res) {
  ...
  if (!email || !userId) return res.status(400).json({ error: "Se requiere email y userId." });
  ...
  if (!priceId) return res.status(500).json({ error: `Price ID no configurado para plan "${selectedPlan}".` });
  ...
  try {
    ...
    if (existing.data.length > 0) {
      ...
      await stripe.customers.update(customer.id, { metadata: { firebaseUID: userId, plan: selectedPlan } });
    } else {
      customer = await stripe.customers.create({ email, metadata: { firebaseUID: userId, plan: selectedPlan } });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: { metadata: { userId, plan: selectedPlan, firebaseUID: userId } },
